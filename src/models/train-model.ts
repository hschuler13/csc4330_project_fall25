...
interface IssueData {
  ...
  resolver_commit_count_before_resolution: number | null;
  is_newcomer_resolver: boolean | null;
  has_gfi_label: boolean;
  resolution_pr_number: number | null;
  resolution_method: 'pr' | 'commit' | 'unknown' | null;
...
interface RepoStats {
  ...
  created_at: string;
  closed_gfi_count: number;
}

class GFIScraperGraphQL {
  // === Storage: deduped via Map ===
  private closedIssuesMap: Map<string, IssueData> = new Map();
  private issueKey = (owner: string, repo: string, num: number) => `${owner}/${repo}#${num}`;

  private repoStats: Map<string, RepoStats> = new Map();
  private outputDir: string = './scraped_data_graphql';

  // For has_gfi_label feature only (we now query only the exact label)
  private gfiLabelVariations = [
    'good first issue',
    'good-first-issue',
    'goodfirstissue',
    'beginner',
  ...
  // === Caches ===
  private userIdByLogin = new Map<string, string>(); // login -> userId
  private commitCountCache = new Map<string, number>(); // `${owner}/${repo}|${authorId}|${untilISO}` -> count
  private prHumanResolver = new Map<string, string | null>(); // `${owner}/${repo}#PR` -> human login or null

  // Bot detection
  private KNOWN_BOTS = new Set(["dependabot[bot]", "github-actions[bot]", "renovate[bot]", "mergify[bot]"]);
  private isBotLogin = (login?: string) =>
    !!login && (login.endsWith("[bot]") || this.KNOWN_BOTS.has(login));
  ...
  private getClosedIssuesArray(): IssueData[] {
    return Array.from(this.closedIssuesMap.values());
  }

  // Main scraping orchestrator
  async scrapeAllRepos(): Promise<void> {
  ...
  async scrapeRepository(owner: string, repo: string): Promise<void> {
  ...
  async getClosedGFIIssues(owner: string, repo: string): Promise<any[]> {
    const allIssues: any[] = [];
    ...
    while (hasNextPage) {
      try {
        const result: any = await graphqlWithAuth(query, { owner, repo, cursor });
  ...
  async processClosedIssue(owner: string, repo: string, issue: any): Promise<IssueData | null> {
    try {
      ...
      const hasGFILabel = labels.some((label: string) =>
        this.gfiLabelVariations.some(v => label.toLowerCase().includes(v.toLowerCase()))
      );
      ...
      if (resolverInfo?.resolver) {
        ...
        const contributorStats = await this.getContributorStats(
        owner, repo, resolverInfo.resolver, issue.closedAt,
        resolverInfo?.pr_number != null ? { prNumber: resolverInfo.pr_number } : undefined
        );
  ...
  async findResolverFromTimeline(owner: string, repo: string, issue: any): Promise<{
    resolver: string;
    pr_number: number | null;
    method: 'pr' | 'commit' | 'unknown';
    ...
    const referencingPRs = timelineItems
      .filter((it: any) =>
        it.__typename === 'CrossReferencedEvent' &&
  ...
  async getLinkedPullRequests(owner: string, repo: string, issueNumber: number): Promise<any[]> {
    const query = `
    ...
    `;
    try {
      const result: any = await graphqlWithAuth(query, { owner, repo, issueNumber });
      const prs: any[] = [];
  ...
  async getContributorStats(
    ...
    repo: string,
    username: string,
    beforeDate: string, // ISO closedAt
    opts?: { prNumber?: number }
  ): Promise<{
    commitCount: number | null; // null if skipped due to bot/unknown
    prCount: number;
    ...
    `;
    try {
      const histRes: any = await graphqlWithAuth(historyQuery, {
        owner, repo, authorId, until
      });
      const totalCount: number =
        histRes.repository?.defaultBranchRef?.target?.history?.totalCount ?? 0;
  ...
  private async mapBotPRToHuman(owner: string, repo: string, prNumber: number): Promise<string | null> {
    const key = `${owner}/${repo}#${prNumber}`;
    if (this.prHumanResolver.has(key)) return this.prHumanResolver.get(key)!;
    ...
    `;
    try {
      const res: any = await graphqlWithAuth(q, { owner, repo, number: prNumber });
      ...
      for (const n of nodes) {
        ...
        if (a && !this.isBotLogin(a)) humans.add(a);
        if (c && !this.isBotLogin(c)) humans.add(c);
      }
  ...
  private async getUserId(login: string): Promise<string | null> {
    if (this.userIdByLogin.has(login)) return this.userIdByLogin.get(login)!;
    const q = `query ($login: String!) { user(login: $login) { id } }`;
    try {
      const res: any = await graphqlWithAuth(q, { login });
      const id = res?.user?.id ?? null;
      if (id) this.userIdByLogin.set(login, id);
      return id;
    } catch (e) {
  ...
  private async getUserPRCount(login: string): Promise<number> {
    const q = `
      query ($login: String!) {
        user(login: $login) {
  ...
  private saveTrainingCSV(filepath: string): void {
    const headers = [
      'repo_owner',
      'repo_name',
      'issue_number',
